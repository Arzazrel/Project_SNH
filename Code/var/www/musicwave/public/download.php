...
if (empty($user_csrf_token) || !hash_equals($_SESSION['csrf_token'] ?? '', $user_csrf_token)) {
    $securityLogger->warning("CSRF download interception triggered", ["user_id" => $_SESSION['user_id'], "ip" => $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN_IP']);
    header("HTTP/1.1 403 Forbidden");
...
// if the string does not start with a number it will be converted to 0. So in case of XSS injection the value will be 0.
...
// path Traversal Countermeasure: Extract strict filename and anchor directory path on server
...
$absolute_storage_path = DIR_UPLOADS_AUDIO . $isolated_filename;	// set the path in the chosen folder
