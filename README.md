...
│	│	│
│	│	├── stored_media					<-- 
│   │	│	└── uploads						<-- folder containing the files entered into the DB by users
│   │	│		└── audio					<-- audio files
│   │	│
...
│	│
│	│── csrf_test					<-- file to test csrf against the app
│	│
│	└── timing_test					<-- file to check if login is vulnerable to timing attacks
│
...
However, if your PHP environment natively supports **Argon2id** (the current industry state-of-the-art algorithm, which provides superior resistance against GPU/ASIC-hardware parallel cracking and side-channel attacks), the app use this one to upgrade the security layer. 
